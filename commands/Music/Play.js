...
const resolve = await client.poru.resolve(args.join(' '))
    ...
    if (loadType === "PLAYLIST_LOADED") {
      ...
      message.channel.send({ content: `<:huuuuggggblack:1002235931765514310> Added: \`${resolve.tracks.length} from ${resolve.playlistInfo.name}\`` });
    ...
    track.info.requester = message.author;
     ...
     player.queue.add(track);         
        message.channel.send({ content: `<:huuuuggggblack:1002235931765514310> Added: \`${track.info.title}\`` });
        if (!player.isPlaying && !player.isPaused) return  player.play();
